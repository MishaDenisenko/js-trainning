const [quote, author] = document.querySelectorAll('.quote');
const generateBtn = document.querySelector('.quote-bth');
...
const generate = () => {
    let quoteAuthor = authors[Math.floor(Math.random() * authors.length)];

    quote.textContent = `"${QUOTES[quoteAuthor]}"`;
    author.textContent = `- ${quoteAuthor}`;

    ls.setItem('quote', quote.textContent);
    ls.setItem('author', author.textContent);
}
...
window.onload = () => {
    ...
    quote.textContent = ls.getItem('quote');
    author.textContent = ls.getItem('author');
};
